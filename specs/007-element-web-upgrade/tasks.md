...
- [ ] T026 [US1] Find why no inbound Opus frame is decoded on v1.12.25. The negotiation now succeeds and publishing is clean — 16,750 captured, 16,749 sent — but nothing is decoded from the remote tracks, which the SFU does offer (`a=sendonly` with real msids) and which we do subscribe to. **This is what now blocks T008**, and it is a receive-path question rather than a signalling one, so the trace added for T025 will not answer it. **Narrowed already**: the SFU sends *no* subscriber offer at all (0 `offer`, against 420 `subscription_response` and 6 `media_sections_requirement`), so no receive section is ever negotiated. Start at livekit-client's `media_sections_requirement` handler — evidence in `spec.md`
