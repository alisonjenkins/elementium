...
- [X] T012 [US1][US2] **Decided: change nothing in the sending schedule.** Raising the delay buys a shorter silence with a longer window in which someone who has left can still decrypt; acknowledged distribution has nothing to acknowledge it, since Matrix gives the sender no receipt; re-sending at a fresh index restarts the same race. Our own decrypt path has no failure latch, so "I cannot hear others" self-heals when the key lands. The remaining exposure is the peer's client and not ours to fix. See the decision in `spec.md`. Original task follows. Decide what to do about a rotation whose key does not reach a peer within `useKeyDelay`, which is the fault that survives T008. Five seconds is a guess about to-device latency, and when the guess is wrong the peer latches the index and hears nothing until the key lands. A late key does recover the index (see T010), so this is about the length of the gap, not a permanent loss. Options worth costing: raise the delay, defer adoption until distribution is acknowledged, or re-send at a fresh index on suspicion. See the 2026-08-07 finding in `specs/003-call-media-faults/spec.md`
...
- [X] T010 [US3] **Upstream's, in both cases, and neither occurs here.** The `failureTolerance` latch is livekit-client's; our own decrypt path has no equivalent, and installing a key at the latched index clears theirs, so there is nothing for us to work around. The parked-key and out-of-order paths are `matrix-js-sdk`'s and are not reachable from a widget. Measured rather than assumed: 29 keys received, 29 forwarded, 29 installed in the 2026-08-08 run, with none parked or dropped. Original task follows. For any key that dies inside `RTCEncryptionManager`, decide whether it is ours to work around or upstream's to fix, and record which. One answer is already in: livekit's failure-count latch stops retrying a key index after `failureTolerance` failures, but installing a key at that index clears the count, so a late key does revive it. The latch amplifies a persistent failure into permanent silence rather than causing one on its own
