...
- [~] T014 [US1] **Re-assessed 2026-08-08 against six real failures from a live session, and SC-001 does not hold for four of them.** Diagnosable from logs alone: the odd-geometry blank frame (the conversion error names the exact byte-size mismatch) and a shared window disappearing (once that fix added a line at all). Not diagnosable: `error alloc buffers: Invalid argument` never names the buffer-type mismatch behind it; `Failed to initialize VP8 encoder width=0 height=0` blamed the encoder when the fault was upstream (now improved at both layers, `vpx_codec.rs` and `ensure_encoder`); and a receiver decoding nothing logs each PLI but never that no keyframe was ever sent in reply. Structurally, FR-002's correlation id does not reach `elementium-media` or `elementium-codec` at all, so capture and encode events cannot be tied to a call's timeline. The original checkbox was earned by code review on a headless machine, which is why this went unnoticed. **Acted on since**: the buffer-type mismatch is now named on the capture failure event, the VP8 init error names which geometry problem it is (or says it is not one), and each capture pipeline carries a span naming its track so `elementium-media` events are attributable. **Now closed too**: a call's correlation id is recorded when the room connects, adopted by every capture pipeline, and cleared when the pipelines detach — previously `get_user_media` minted a fresh id per invocation, giving one call two unrelated ids. **Where that leaves SC-001**, by reasoning rather than measurement, so read it as an estimate: of the six real failures assessed, five would now lead a maintainer to the cause (the buffer-type mismatch, the encoder's geometry, unanswered keyframe requests, the removed node, the chroma-size error), and the sixth — truncated MJPEG buffers — is a driver fault the logs can now describe but not resolve. Re-assessing against fresh failures, rather than the ones the fixes were written for, is what would actually settle it. Manually validate quickstart.md's Story 1 section end-to-end: trigger a real camera-start failure, confirm every event from `enumerate_devices` through the camera thread's failure shares one `correlation_id`, confirm the failure event's fields are enough to diagnose without reading source (NOT YET DONE — this session's environment is headless/no display, can't drive the Tauri GUI to trigger a real getUserMedia call; the span-inheritance wiring was verified by code review + the T006 startup-log check, but a real failure trigger needs a machine with a display/camera)
...
- [X] T016 [US2] Implement the custom `tracing_subscriber::Layer`-based log-capture fixture (per research.md's Test-capture fixture decision) — `Vec<Mutex<CapturedEvent>>`-backed, `on_event` hook records name/level/fields — as a small internal crate `crates/elementium-observability-test/` or a shared `#[cfg(test)]` module reachable from both `elementium-webrtc` and `src-tauri` (pick whichever avoids duplicate code; if a shared crate, add it to `[workspace.dependencies]`/members in `/home/ali/git/elementium/Cargo.toml` as `[dev-dependencies]` for consumers)
- [X] T017 [US2] [P] Add assertion helpers to the fixture from T016: `find_event(name) -> Option<&CapturedEvent>`, `CapturedEvent::field(key) -> Option<&FieldValue>`, and a way to assert a field's string/bool/int value without manual `HashMap` lookups in every test
- [X] T018 [US2] Write regression test 1 in `crates/elementium-webrtc/src/` (test module near `pc_io_loop`/`encrypt_or_drop`): simulate `encrypt_frame` returning `None` (no key set) on the outbound frame-write path, run it under the T016 fixture via `tracing::subscriber::with_default`, and assert a structured "frame dropped" warning event was emitted with a `reason` field — confirm it fails if `encrypt_or_drop`'s drop-and-warn logic is reverted to `unwrap_or(data)`
- [X] T019 [US2] Write regression test 2 near `resample_44100_to_48000` in `src-tauri/src/commands/media_devices.rs`: call it with `channels = 0` under the T016 fixture, assert (a) no panic (existing `channels.max(1)` guard holds) and (b) a structured event fires noting the anomalous zero-channels input was clamped — confirm it fails if that event is removed
- [X] T020 [US2] [P] Write a dedicated secret-redaction test using the T016 fixture: exercise `elementium-e2ee`'s key-set path and `elementium-keyring`'s secret-access path under capture, then assert no captured event's fields contain the literal secret/key value (only presence/absence booleans) — satisfying SC-005's "automated scan" requirement structurally rather than via grep
- [X] T021 [US2] Run `cargo clippy --workspace --all-targets` and `cargo test --workspace` inside `nix develop`; confirm T018-T020 pass, then temporarily revert each underlying fix one at a time to confirm each test fails as expected (manual verification step per quickstart.md, not a permanent code change)
