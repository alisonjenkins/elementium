...
- [ ] T018 [US2] `just app-join` reached a call once and has not since: Element Web shows "There's no preview, would you like to join?" for a room the homeserver reports the account joined, and `/sync` fails. Reproduced on a freshly wiped homeserver, so it is not accumulated device or room state. The sync error itself was `{}` in the log until the console bridge was fixed; the next run should say what it is. Everything else about the harness works — the app starts headless, in its own profile, seeds a session and drives the UI
