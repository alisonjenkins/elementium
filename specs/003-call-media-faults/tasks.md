...
- [X] T013 [US2] Two-participant call: confirm video is seen both ways. **Done**: with two remote publishers both tracks deliver frames, confirmed on two runs, alongside 1,028 inbound Opus frames
- [X] T014 [US1][US2][US3] Confirm the logs show zero decrypt failures and zero closed-channel drops for a healthy call. **Zero of each**, over a three-minute call with two peers: 7,250 of 7,250 captured frames encoded and sent, 14,500 inbound Opus frames decoded, `dropped_channel_closed: 0`, `dropped_channel_full: 0`, `encode_errors: 0`, and no decrypt failure of any kind. Both peers heard 2/2 streams throughout and held all three keys. One warning survives and is described below
- [X] T015 [US1] `crates/elementium-webrtc/tests/livekit_local_roundtrip.rs` hung indefinitely against the local stack. **Not a broken instrument after all**: the test body always passed — audio really does go publish → SFU → subscribe → decode — and the hang was tokio refusing to shut down while two `spawn_blocking` loops that can never return were still running. Both are product code. Now passes in 38s; see the finding below
...
- [X] T018 [US2] `just app-join` reached a call once and then never again. **Two causes, both in the harness**: Element Call refused to start because nothing answers well-known discovery at `https://localhost/` (already solved for Playwright, never for the app), and the autojoin injection was skipped when already present — so every run after the first pointed the app at the *previous* run's room and token
- [X] T017 [US2] Find why the second remote video track is polled once and then abandoned. **Does not reproduce**: both tracks deliver frames on every run since the harness was fixed. The single earlier observation was most likely a peer that had not started publishing when the throttled counter sampled it. The fetch loop now reports stops and failures, so a recurrence will identify itself
